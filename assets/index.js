...
const TestIf = () => {
  let $count = stream(0);
  return h.div({}, [
    h.button({ onclick: () => $count.val++ }, [
      "Click me"
    ]),
    h.div({'data-test': "hi", 'data-if': () => $count.val > 0 }, [
      h.span({ "data-text": () => $count.val }),
      h.span({ "data-if": (() => $count.val % 2 !== 0)}, [" Count is odd"]),
      h.span({ "data-if": (() => $count.val % 2 == 0)}, [" Count is even"]),
      h.br(),
      h.button({ onclick: () => console.log('clicked') }, [
        h.span({}, [
          h.span({ 'data-text': () => {
            ...
            ? 'buzz' 
            : 'hi' }}),
          h.span({'data-if': () => $count.val % 5 === 0 && $count.val % 3 === 0}, [" fizzBuzz"])
        ])
      ]),
    ]),
